...
async def extract_audio_chunks(url: str, chunk_seconds: int = 6):
    """
    Use yt-dlp to extract audio from a livestream URL and yield it in chunks.

    Args:
        url: YouTube or Facebook live stream URL.
        chunk_seconds: Duration of each chunk in seconds.

    Yields:
        Paths to temporary audio files containing chunk_seconds of audio.

    Note: This function spawns yt-dlp as a subprocess.  It writes audio
    chunks to temporary files (WAV format).  Proper cleanup is performed
    automatically.
    """
    if yt_dlp is None:
    ...
    # spawn yt-dlp process to extract audio
    cmd = ['yt-dlp', '-f', 'bestaudio', '-o', '-', url]
    proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.PIPE)

    # Use ffmpeg via subprocess to segment audio stream into fixed-length WAV files
    # We'll call ffmpeg to read from stdin and write chunked WAV files to tmp_dir.
    ...
    # Wait for ffmpeg to produce chunk files asynchronously
    chunk_index = 0
    try:
        while True:
            ...
            if not chunk_path.exists():
                # No new chunk; check if ffmpeg is still running
        ...
        if ffmpeg_proc.returncode is None:
            ffmpeg_proc.kill()
        # Remove tmp_dir (but not while iterating; outside of generator) – left for caller
        pass


def transcribe_file(file_path: Path) -> str:
    """Transcribe Spanish audio to Spanish text using Whisper."""
    global asr_model
    ...
    # Use Whisper to transcribe; specify language 'es' (Spanish)
...
async def process_stream(url: str):
    """
    Extract audio, transcribe and translate it, and broadcast subtitles via WebSocket.

    This coroutine continuously reads audio chunks from the livestream URL,
    produces English translations with timestamps, and broadcasts them to
    all connected WebSocket clients.
    """
    logging.info("Starting stream processing for %s", url)
    chunk_generator = extract_audio_chunks(url)
    subtitle_index = 0
    async for chunk_path in chunk_generator:
        # Transcribe Spanish audio
        spanish = transcribe_file(chunk_path)
        # Translate to English
        english = translate_spanish_to_english(spanish)
        # Create subtitle payload with basic timestamp info
        payload = {
...
async def home(request: Request):
    """Render the homepage with form to start translation."""
    return templates.TemplateResponse('index.html', {'request': request})


@app.get('/pricing', response_class=HTMLResponse)
async def pricing(request: Request):
    """Render a simple pricing page."""
    return templates.TemplateResponse('pricing.html', {'request': request})


@app.get('/admin', response_class=HTMLResponse)
async def admin(request: Request):
    """Render an admin page showing active connections."""
    data = {
...
async def websocket_endpoint(websocket: WebSocket):
    """
    WebSocket endpoint that accepts a stream URL and sends live subtitles.

    Client workflow:
        1. Connect to /ws
        2. Send a JSON message: {"url": "<livestream_url>"}
        3. The server starts processing the stream and sends back subtitle
    ...
    try:
        ...
        if not url:
            ...
            return
        # Launch background task to process the stream
